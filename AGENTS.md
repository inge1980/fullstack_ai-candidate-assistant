...
| `Questions/AnswerPromptFormatter.cs` | Shared LLM context/prompt fill used by API and CandidateConsoleAssistant. Context includes Organization, Period (`from` to `to`, plus whole years when the span is at least 24 months), Environment, Technologies, Match (exact vs related), and http(s) Links as ready Markdown (GitHub/live/portfolio). |
