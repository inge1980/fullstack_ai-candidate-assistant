...
| `Questions/QuestionService.cs` | Orchestrates retrieve / prompt / LLM / source URLs. Rule-based `QuestionIntentDetector` runs on the original question. `filter-list` with N uses a wider retrieve window; `filter-list`/`count` without N keep the 25-hit window; both send one chunk per project. |
...
| `Questions/PromptContextSelector.cs` | Default top-10 chunks; `filter-list`+N: wider retrieve, one chunk per `source`, take N; `filter-list`/`count` without N: one chunk per `source` in the 25-hit window |
...
- API default retrieve-25 / prompt-10 vs console retrieve-10. `filter-list` with N and `filter-list`/`count` without N diversify by `source` and do not yet filter on period or technologies.
